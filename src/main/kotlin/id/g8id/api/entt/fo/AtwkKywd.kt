...
import id.g8id.api.cnst.KywdGrup
...
class AtwkKywd : PanacheMongoEntity {
  ...
  companion object: PanacheMongoCompanion<AtwkKywd> {
     ...
     */
    fun addSrchKywd(kywdList: List<String>) {

      // Upsert (Merge)
      val id       = ObjectId(TimeUtil.getTodayYyyyMMdd() + KywdGrup.SEARCH)
      var item     = findById(id)

      println("kywdList $kywdList")

      if (item==null) {
        // 1. if not found, insert one with count 1
        ...
        // 2. if found id, update each keyword count +1
      ...
      println("item " + GsonExt.getGson().toJson(item))
    }
